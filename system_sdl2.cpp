...
void System_SDL2::init(const char *title, int w, int h, bool fullscreen, bool widescreen, bool yuv) {
	...
	if (SDL_InitSubSystem(SDL_INIT_HAPTIC) < 0){
		fprintf(stdout, "SDL force feedback initialisation failed (non-fatal): %s\n", SDL_GetError());
	}
	else if (SDL_NumHaptics() > 0){
		_hapticDevice = SDL_HapticOpen(0);
		if (_hapticDevice == NULL){
			fprintf(stdout,"SDL_HapticOpen(0) failed (non-fatal): %s\n", SDL_GetError());
		}
		else if (SDL_HapticRumbleInit(_hapticDevice) != 0){
			fprintf(stdout,"SDL_HapticRumbleInit failed (non-fatal): %s\n", SDL_GetError());
			SDL_HapticClose(_hapticDevice);
			_hapticDevice = NULL;
		}
		if (SDL_HapticRumblePlay(_hapticDevice, 0.33f /* Strength */, 10 /* Time */) < 0){
			fprintf(stdout,"SDL_HapticRumbleStop failed: %s\n", SDL_GetError());
		}
	} 
}
...
void System_SDL2::updateScreen(bool drawWidescreen) {
	...
	if (!_widescreenTexture) {
		...
		if (_shakeDx > 0) {
			...
			dst += _shakeDx;
			if (_hapticDevice){
				if (SDL_HapticRumblePlay(_hapticDevice, 0.33f /* Strength */, 500 /* Time */) < 0){	
					fprintf(stdout, "SDL_HapticRumblePlay failed: %s\n", SDL_GetError());
				}
			}
		} else if (_shakeDx < 0) {
			w += _shakeDx;
	...
	if (_widescreenTexture) {
		...
		SDL_RenderCopy(_renderer, _texture, &s, &r);						
	}
		...
		case SDL_CONTROLLERBUTTONUP:
			if (_controller) {
				...
				case SDL_CONTROLLER_BUTTON_A:
					if (pressed) {
						pad.mask |= SYS_INP_JUMP|SYS_INP_RUN;
					} else {
						pad.mask &= ~SYS_INP_JUMP|SYS_INP_RUN;
					}
					break;
...
void System_SDL2::setupDefaultKeyMappings() {
	...
	// Just PC
	addKeyMapping(SDL_SCANCODE_Z,        SYS_INP_SHOOT | SYS_INP_RUN);
	addKeyMapping(SDL_SCANCODE_X,        SYS_INP_JUMP);
	// RG-350	
	addKeyMapping(SDL_SCANCODE_SPACE,    SYS_INP_SHOOT | SYS_INP_RUN);//Y
	addKeyMapping(SDL_SCANCODE_LSHIFT,   SYS_INP_SHOOT | SYS_INP_RUN);//X
	addKeyMapping(SDL_SCANCODE_LCTRL,    SYS_INP_JUMP);//A
	addKeyMapping(SDL_SCANCODE_LALT,     SYS_INP_JUMP);//B	
}
